...
- Gitリポジトリコンテキスト
- `gh-retrospective.sh`（closeのみ、~/.local/bin/）
...
### create サブコマンド
1. **ラベル検証**: 利用可能なラベルを取得し、推論ラベルを検証
   ```bash
   gh label list --json name --jq '.[].name'
   ```
2. **ラベル自動推論**: タイトル+本文から推論（検証済みラベルのみ使用）
3. **タスク数確認**: 8個以上で警告表示（分割推奨）
4. **GitHub Issue作成**: `gh issue create`
5. **GitHub Projects追加**: `gh_projects_set_todo`
6. **結果表示**: Issue番号とURL
...
5. checkpoint削除: `delete_memory("issue_{number}_checkpoint")`
