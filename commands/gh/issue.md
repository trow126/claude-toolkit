...
2. **issue-todowrite-sync skill**: 未完了タスクのみTodoWriteに変換
3. **セッションコンテキスト保存**: Issue-TodoWriteマッピング
4. **GitHub Projects更新**: ステータス → "In Progress"
5. **progress-tracker skill**: バックグラウンド監視開始（自動同期、完了時自動クローズ）

**スマートデフォルト**:
- **8タスク以上 + Phase情報あり**: Phase単位で作成
- **それ以外**: 全タスク作成
...
- Task tool（agent委譲用）
- Python 3.6+（Skills用）
- Skills: `issue-parser`, `issue-todowrite-sync`, `progress-tracker`, `issue-retrospective`
...
2. issue-todowrite-sync skill: "use issue-todowrite-sync skill to convert Issue #[number] to TodoWrite"
   - スマートデフォルト（デフォルト）: 8タスク以上+Phase情報 → Phase単位
   - `--all-tasks`: 全タスク強制表示
   - `--phase-by-phase`: Phase単位強制
3. GitHub Projects更新: `scripts/gh-projects-integration.sh` → `gh_projects_set_in_progress`
4. progress-tracker skill: "use progress-tracker skill for Issue #[number]"
```
...
5. セッションコンテキストクリーンアップ
6. ブレインストーミングファイル削除（`--from-file`使用時）
