...
### Phase 3: User Decision（インタラクティブ）

```yaml
10. 各指摘を表示して確認:

    ┌─────────────────────────────────────────────────┐
    │ 📝 指摘 1/6: src/data/realtime.py:276           │
    │                                                 │
    │ 🏷️ Severity: 🟡 Minor (Potential issue)         │
    │ 📂 Category: Bug                                │
    │                                                 │
    │ 💬 指摘内容:                                    │
    │ asyncio.create_task の参照を保持する必要あり    │
    │ タスクがGCされる可能性があります                │
    │                                                 │
    │ 📋 Issue #8 との整合性: ✅ 合致                 │
    │    理由: 接続安定性要件に直接関連              │
...
11. 却下の場合はCodeRabbitに返信:
    gh pr comment <number> --body "
    @coderabbitai

    指摘 \`realtime.py:276\` について:
...
💡 **Trivial は後回し**: Minor/Major を先に対応、Trivial は余裕があれば
